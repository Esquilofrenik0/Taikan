...
namespace SRPG {
  public class ImageColorPicker : MonoBehaviour, IPointerClickHandler {
    ...
    public void OnPointerClick(PointerEventData eventData) {
      selectedColor = GetColor(GetPointerUVPosition(eventData));
    ...
    Vector2 GetPointerUVPosition(PointerEventData eventData) {
      ...
      float uvX = (eventData.position.x - imageCorners[0].x) / texWidth;
      float uvY = (eventData.position.y - imageCorners[0].y) / texHeight;
